...
from typing import Annotated, Callable, Dict, List, Union, Literal

import pathlib

from pydantic import BaseModel, Field, RootModel, StringConstraints, BeforeValidator, model_validator
...
class Folder(RootModel[Dict[Identifier, List[Union[File, "Folder"]]]]):
    ...
    def validate_folder_contents(self):
        ...
        if len(folders) == 0 and len(files) == 0:
            raise ValueError(
                f"Folder must contain at least one file or one subfolder."
            )   
