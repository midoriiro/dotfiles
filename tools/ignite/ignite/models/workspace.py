...
from ignite.models.projects import Projects, RepositoryProject, ReservedProjectKey, UserProject
...
class WorkspaceFileSpecification(BaseModel):
    ...
    """
    folders: List[WorkspaceFolderSpecification] = Field(..., description="The folders in the file")
    settings: Dict[str, Any] = Field(..., description="The settings in the file", default_factory=dict)
...
class Workspace(BaseModel):
    ...
    def resolve_project_folders(self, path_resolver: PathResolver) -> Dict[str, List[ResolvedFolder]]:
        """
        ...
        for project_name, project_resolved_folders in self.projects.resolve_project_folders().items():
        ...
        if ref_project:
            ...
            ref_paths = [pathlib.Path(resolved_folder) for resolved_folders in ref_project for resolved_folder in resolved_folders.sources]
        else:
            ref_paths = None
    ...
    def resolve_file_specification(self) -> WorkspaceFileSpecification:
        ...
        """
        workspace_file_specification: WorkspaceFileSpecification = WorkspaceFileSpecification(
            folders=[],
            settings={}
        )
        for project_name, project in self.projects.root.items():
            if isinstance(project, UserProject):
                workspace_file_specification.folders.append(WorkspaceFolderSpecification(
                    path=str(pathlib.Path(project.path, project_name)),
                    name=project.alias if project.alias else project_name
                ))
            elif isinstance(project, RepositoryProject) and project_name == ReservedProjectKey.ROOT:
                workspace_file_specification.folders.append(WorkspaceFolderSpecification(
                    path=".",
                ))
