...
class Env(Feature):
    ...
    type: Optional[EnvType] = Field(
        EnvType.REMOTE, 
        description="Environment variable type (remote or container)"
    )
...
class Mount(BaseModel):
    ...
    options: Optional[List[MountOption]] = Field(None, description="Additional mount options")
    ...
    def compose(self) -> Dict[str, Any]:
        ...
        if self.user:
            if isinstance(self.user, Users):
                ...
                composed.update(Users
                    .model_construct(remote=self.user, container=self.user)
                    .compose()
                )
        if self.env:
            env_data = {}
...
class Socket(BaseModel):
    ...
    """
    host: AbsolutePath = Field(
        ..., 
        description="Host socket path (absolute path)"
    )
...
class Expose(Feature):
    ...
    def compose(self) -> Dict[str, Any]:
        ...
        if self.address:
            composed.update(Env(key="CONTAINER_HOST", value=str(self.address)).compose())
        return composed
...
class Extensions(Feature):
    ...
    vscode: List[Extension] = Field(
        ...
        min_length=1,
        max_length=100,
        description="List of vscode extensions"
    )
    ...
    def compose(self) -> Dict[str, Any]:
        ...
        composed = {
            "customizations": {
                "vscode": {
                    "extensions": [extension for extension in self.vscode]
                }
            }
        }
        return composed
