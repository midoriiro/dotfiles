        ...
        for key, value in self.root.items():
            if key == ReservedProjectKey.ROOT:
                if not isinstance(value, RepositoryProject):
                    raise ValueError(f"The value for key '{ReservedProjectKey.ROOT.value}' must be a RepositoryProject instance.")

            if key == ReservedProjectKey.REF:
                if not isinstance(value, RepositoryProject):
                    raise ValueError(f"The value for key '{ReservedProjectKey.REF.value}' must be a RepositoryProject instance.")
    ...
    def resolve_project_folders(self) -> Dict[str, List[ResolvedFolder]]:
        ...
        for key, value in self.root.items():
            if isinstance(value, UserProject):
                ...
                for resolved_folder in resolved_folders:
                    resolved_folder.destination = str(pathlib.Path(value.path, key, resolved_folder.destination))
