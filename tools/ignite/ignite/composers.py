...
class Composer:
    ...
    def _ask_create_folder(self, output_path: Path) -> bool:
        ...
        """
        return typer.confirm(f"Folder '{output_path}' does not exist. Do you want to create it?")
    ...
    def _ask_overwrite_file(self, output_path: Path) -> bool:
        ...
        """
        return typer.confirm(f"File '{output_path}' already exists. Do you want to overwrite it?")
    ...
    def _save_file(
        ...
        if not base_path.exists():
            if folder_policy == FolderCreatePolicy.ASK:
                ...
                raise ValueError(f"Folder '{base_path}' does not exist and policy is set to never.")
            else:
        ...
        if output_path.exists():
            if file_policy == FileWritePolicy.ASK:
                ...
                raise ValueError(f"File '{output_path}' already exists and policy is set to never.")
...
class WorkspaceComposer(Composer):
    ...
    def _resolve_project_files(self) -> List[ResolvedFile]:
        ...
        resolved_project_folders = self.__workspace.resolve_project_folders(self.__path_resolver)
        for project_name, resolved_folders in resolved_project_folders.items():
            ...
            for resolved_folder in resolved_folders:
                resolved_project_files.append(resolved_folder.resolve(file_content_variables))
    ...
    def _resolve_file_specification(self) -> ResolvedFile:
        ...
        return ResolvedFile(
            ...
            content=json.dumps(
                self.__workspace.resolve_file_specification().model_dump(exclude_none=True),
                indent=JSON_INDENT
            )
        )
