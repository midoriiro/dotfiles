...
from ignite.models.fs import ResolvedFolder, FileTemplateVariables, ResolvedFile
...
def json_source_files(user_context: Path) -> Dict[str, Path]:
    ...
    data1 = {"name": "${project-name}", "version": "1.0.0", "dependencies": {"requests": "2.28.0"}}
    file1.write_text(json.dumps(data1, indent=2))
    files["source1"] = file1
    
    # Second JSON file
    file2 = user_context / "source2.json"
    data2 = {"version": "2.0.0", "dependencies": {"pytest": "7.0.0"}, "scripts": {"test": "pytest"}}
...
class TestResolvedFolderResolveFormats:
    ...
    def test_resolve_json_files(self, json_source_files: Dict[str, Path], template_variables: FileTemplateVariables):
        ...
        assert_that(content_data["version"]).is_equal_to("2.0.0")  # Second file overwrites first
    ...
    def test_resolve_yaml_files(self, yaml_source_files: Dict[str, Path], template_variables: FileTemplateVariables):
    ...
    def test_resolve_ini_files(self, ini_source_files: Dict[str, Path], template_variables: FileTemplateVariables):
        ...
        # Check that data was merged correctly
        assert_that(config["DEFAULT"]["name"]).is_equal_to(template_variables.project_name)
        assert_that(config["DEFAULT"]["version"]).is_equal_to("2.0.0")  # Second file overwrites first
    ...
    def test_resolve_with_yml_extension(self, yaml_source_files: Dict[str, Path], template_variables: FileTemplateVariables):
...
class TestResolvedFolderResolveScenarios:
    ...
    def test_resolve_with_single_source(self, json_source_files: Dict[str, Path], template_variables: FileTemplateVariables):
...
class TestResolvedFolderErrorHandling:
    ...
    def test_resolve_with_unsupported_extension(self, json_source_files: Dict[str, Path], template_variables: FileTemplateVariables):
