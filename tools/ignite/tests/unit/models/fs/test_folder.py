...
from ignite.models.fs import Folder, File, ReservedFileName
...
class TestFolderValidation:
    ...
    def test_folder_must_contain_at_least_one_file(self):
        ...
        with pytest.raises(ValueError, match="Folder must contain at least one file or one subfolder"):
            Folder({
                "empty": []
            })
...
class TestReservedKeywordsValidation:
    ...
    def test_all_cannot_be_defined_more_than_once(self):
        ...
        with pytest.raises(ValueError, match="'\\$all' cannot be defined more than once"):
            Folder({
                "src": [
                    File("$all"),
                    File("$all")
    ...
    def test_ref_and_all_cannot_be_defined_simultaneously(self):
        ...
        with pytest.raises(ValueError, match="'\\$ref' and '\\$all' cannot be defined simultaneously"):
            Folder({
                "src": [
                    File("$ref"),
                    File("$all")
    ...
    def test_all_cannot_be_defined_with_repository_files(self):
        ...
        with pytest.raises(ValueError, match="'\\$all' cannot be defined simultaneously with repository files"):
            Folder({
                "src": [
                    File("$all"),
                    File("config")
...
class TestFolderResolve:
    ...
    def test_resolve_with_ref_reserved_keywords(self):
        ...
        paths = folder.resolve()
        expected_paths = [
            pathlib.Path("src", "$ref"),
            pathlib.Path("src", "config")
        ]
        assert_that(paths).is_equal_to(expected_paths)
