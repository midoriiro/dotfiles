...
class TestValidProjects:
    ...
    def test_minimal_projects_with_user_project(self):
        ...
        projects = Projects({"test-project": UserProject(path="tools")})
    ...
    def test_projects_with_user_project_with_alias(self):
        ...
        projects = Projects(
            {"project-key": UserProject(path="tools", alias="my-alias")}
        )
    ...
    def test_projects_with_repository_project_with_vscode(self):
        ...
        projects = Projects({"root": RepositoryProject(vscode=vscode_config)})
        assert_that(projects.root).contains_key("root")
...
class TestProjectsValidation:
    ...
    def test_root_project_with_wrong_type_raises_error(self):
        ...
        with pytest.raises(
            ValueError,
            match="The value for key 'root' must be a RepositoryProject instance",
        ):
            Projects({"root": UserProject(path="tools")})

    def test_ref_project_with_wrong_type_raises_error(self):
        """Test that ref project with wrong type raises validation error."""
        with pytest.raises(
            ValueError,
            match="The value for key 'ref' must be a RepositoryProject instance",
        ):
            Projects({"ref": UserProject(path="tools")})
...
class TestProjectsResolveProjectFolders:
    ...
    def test_resolve_project_folders_with_user_project_with_alias(self):
        ...
        projects = Projects(
            {
                "project-key": UserProject(
                    path="tools", alias="my-alias", vscode=vscode_config
                )
            }
        )
        result = projects.resolve_project_folders()
        ...
        assert_that(resolved_folder.destination).is_equal_to(
            str(pathlib.Path("tools", "project-key", ".vscode", "settings.json"))
        )
        ...
        projects = Projects(
            {
                ...
                "user-project": UserProject(path="tools", vscode=vscode_config),
            }
        )
        result = projects.resolve_project_folders()
    ...
    def test_resolve_project_folders_with_multiple_user_projects(self):
        ...
        projects = Projects(
            {
                "project1": UserProject(path="tools", vscode=vscode_config),
                "project2": UserProject(
                    path="tools", alias="my-project", vscode=vscode_config
                ),
            }
        )
        result = projects.resolve_project_folders()
...
class TestProjectsEdgeCases:
    ...
    def test_projects_with_user_project_no_alias(self):
        ...
        vscode_config = VSCodeFolder(settings=[Folder({"python": [File("base")]})])
        projects = Projects(
            {"project-key": UserProject(path="tools", vscode=vscode_config)}
        )
        result = projects.resolve_project_folders()
    ...
    def test_projects_with_user_project_empty_alias(self):
        ...
        with pytest.raises(ValidationError):
            Projects(
                {
                    "project-key": UserProject(
                        path="/workspace/project", alias="", vscode=vscode_config
                    )
                }
            )
...
class TestProjectsReservedKeys:
    ...
    def test_projects_with_root_key(self):
        ...
        projects = Projects({ReservedProjectKey.ROOT: RepositoryProject()})
    ...
    def test_projects_with_ref_key(self):
        ...
        projects = Projects({ReservedProjectKey.REF: RepositoryProject()})
