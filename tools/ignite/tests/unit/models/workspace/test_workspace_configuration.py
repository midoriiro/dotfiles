...
class TestWorkspace:
        ...
        projects = Projects(
            {
                ...
                "another-project": UserProject(path="tools", alias="custom-alias"),
            }
        )
    ...
    def test_workspace_with_repository_projects(self):
        ...
        assert_that(workspace.projects.root[ReservedProjectKey.ROOT]).is_instance_of(
            RepositoryProject
        )
        assert_that(workspace.projects.root[ReservedProjectKey.REF]).is_instance_of(
            RepositoryProject
        )
...
class TestWorkspaceResolveFileSpecification:
        ...
        projects = Projects(
            {
                ...
                "project2": UserProject(path="tools", alias="custom-alias"),
            }
        )
    ...
    def test_resolve_file_specification_with_mixed_projects(self):
        ...
        assert_that(user_folder.path).is_equal_to(
            str(pathlib.Path("tools", "user-project"))
        )
...
class TestWorkspaceFolderSpecification:
    ...
    def test_workspace_folder_specification_creation(self):
        ...
        folder_spec = WorkspaceFolderSpecification(
            path="/workspace/project", name="test-project"
        )
...
class TestWorkspaceFileSpecification:
    ...
    def test_workspace_file_specification_creation(self):
        ...
        file_spec = WorkspaceFileSpecification(
            folders=[folder_spec], settings={"key": "value"}
        )
