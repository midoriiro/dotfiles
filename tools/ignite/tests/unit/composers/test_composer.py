...
class TestComposerSaveFile:
    ...
    def test_save_file_raises_error_for_directory_path(self, tmp_path):
        ...
        assert_that(composer._save_file).raises(ValueError).when_called_with(dir_path, "test content").is_equal_to(f"Path '{dir_path}' is a directory.")
    ...
    def test_save_file_saves_content_when_file_does_not_exist(self, tmp_path, caplog):
        ...
        assert_that(any("File '{}' saved.".format(output_path) in record.message for record in caplog.records)).is_true()
        
        # Verify content was saved
        assert_that(output_path.read_text()).is_equal_to("test content")

    def test_save_file_creates_parent_directories_when_they_dont_exist(self, tmp_path, caplog):
        ...
        assert_that(any("Folder '{}' created.".format(output_path.parent) in record.message for record in caplog.records)).is_true()
...
class TestComposerFolderPolicies:
    ...
    @patch('typer.confirm')
    def test_ask_policy_when_user_confirms_folder_creation(self, mock_confirm, tmp_path, caplog):
        ...
        mock_confirm.assert_called_once_with("Folder '{}' does not exist. Do you want to create it?".format(output_path.parent))
    ...
    @patch('typer.confirm')
    def test_ask_policy_when_user_declines_folder_creation(self, mock_confirm, tmp_path, caplog):
        ...
        assert_that(any("Folder '{}' creation skipped.".format(output_path.parent) in record.message for record in caplog.records)).is_true()
    ...
    def test_never_policy_when_folder_does_not_exist(self, tmp_path):
        ...
        assert_that(composer._save_file).raises(ValueError).when_called_with(
            output_path, 
            "test content",
            folder_policy=FolderCreatePolicy.NEVER
        ).is_equal_to(f"Folder '{output_path.parent}' does not exist and policy is set to never.")
...
class TestComposerFilePolicies:
    ...
    @patch('typer.confirm')
    def test_ask_policy_when_user_confirms_overwrite(self, mock_confirm, tmp_path, caplog):
        ...
        mock_confirm.assert_called_once_with("File '{}' already exists. Do you want to overwrite it?".format(output_path))
        mock_file.assert_called_once_with(output_path, "w")
        mock_file().write.assert_called_once_with("new content")

    @patch('typer.confirm')
    def test_ask_policy_when_user_declines_overwrite(self, mock_confirm, tmp_path, caplog):
        ...
        assert_that(any("File '{}' skipped.".format(output_path) in record.message for record in caplog.records)).is_true()
    ...
    def test_skip_policy_when_file_exists(self, tmp_path, caplog):
        ...
        composer._save_file(
            output_path, 
            "new content",
            file_policy=FileWritePolicy.SKIP
        )
    ...
    def test_overwrite_policy_when_file_exists(self, tmp_path, caplog):
        ...
        assert_that(any("File '{}' will be overwritten.".format(output_path) in record.message for record in caplog.records)).is_true()
    ...
    def test_never_policy_when_file_exists(self, tmp_path, caplog):
        ...
        assert_that(composer._save_file).raises(ValueError).when_called_with(
            output_path, 
            "new content",
            file_policy=FileWritePolicy.NEVER
        ).is_equal_to(f"File '{output_path}' already exists and policy is set to never.")
