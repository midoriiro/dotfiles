...
class TestWorkspaceComposerInitialization:
    ...
    def test_workspace_composer_initialization(self, minimal_workspace_configuration):
        ...
        assert_that(composer._WorkspaceComposer__workspace).is_equal_to(minimal_workspace_configuration)
        assert_that(composer._WorkspaceComposer__path_resolver).is_equal_to(path_resolver)
...
class TestWorkspaceComposerCompose:
    ...
    def test_compose_with_projects(self):
        ...
        assert_that(resolved_files).is_length(1)  # Only workspace file, no project files
...
class TestWorkspaceComposerResolveFileSpecification:
    ...
    def test_resolve_file_specification_creates_workspace_file(self, minimal_workspace_configuration):
...
class TestWorkspaceComposerResolveProjectFiles:
    ...
    def test_resolve_project_files_with_no_projects(self, minimal_workspace_configuration):
...
class TestWorkspaceComposerSave:
    ...
    def test_save_without_compose_raises_error(self, minimal_workspace_configuration, tmp_path):
    ...
    def test_save_uses_workspace_policies(self, minimal_workspace_configuration, tmp_path, caplog):
        ...
        with patch.object(composer, '_save_file') as mock_save_file:
            ...
            assert_that(call_args[1]['folder_policy']).is_equal_to(FolderCreatePolicy.ALWAYS)
            assert_that(call_args[1]['file_policy']).is_equal_to(FileWritePolicy.OVERWRITE)

    def test_save_creates_directories_as_needed(self, minimal_workspace_configuration, tmp_path):
