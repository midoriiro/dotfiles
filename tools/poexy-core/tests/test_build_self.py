...
def test_prepare_metadata_for_build_wheel(tmp_path, dist_package_name, package_version):
    ...
    assert_that(metadata_name).is_equal_to(f"{dist_package_name}-{package_version}.dist-info")
...
def test_wheel(
    ...
    assert_zip_file(
        [
            wheel_data_purelib_folder(default_python_tag) / dist_package_name() / "api.py",
        ]
    )
