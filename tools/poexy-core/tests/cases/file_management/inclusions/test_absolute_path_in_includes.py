"""
Test case: Absolute path specified in includes configuration

This test verifies that poexy-core properly handles and reports errors when
absolute paths are used in includes sections. All paths in includes should
be relative to the project root for portability and security.

Test scenario:
- A pyproject.toml file contains absolute paths in includes configuration
...
def project_path(sample_project):
    return sample_project(
        SamplePaths.FileManagementInclusions / "absolute_path_in_includes"
    )
