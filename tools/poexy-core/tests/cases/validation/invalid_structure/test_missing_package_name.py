"""
Test case: Missing package name in pyproject.toml configuration

This test verifies that poexy-core properly handles and reports errors when the
package name is missing or undefined in the [project] section of pyproject.toml.
The package name is a required field for all package types.

Test scenario:
- A pyproject.toml file is missing the 'name' field in [project] section
- The build system attempts to determine the package name
...
- The error message clearly indicates that package name is required
- No build artifacts are created when the name is missing
- Both wheel and sdist builds fail consistently with the same error

Edge case significance:
This tests validation of required metadata fields. Package name is fundamental
to Python packaging and must be present for distribution. Proper validation
ensures packages cannot be created without essential identifying information.
"""
...
def project_path(sample_project):
    return sample_project(
        SamplePaths.ValidationInvalidStructure / "missing_package_name"
    )
