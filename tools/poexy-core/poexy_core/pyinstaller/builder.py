...
class PyinstallerBuilder:
    def __init__(
        ...
        if wheel is not None and WheelFormat.Binary not in wheel.format:
            raise PyinstallerBuilderError(
                "Cannot build binary executable for wheel package as specified in [tool.poexy.wheel.format]"
            )
        binary = poexy.binary
        if binary is None:
            raise PyinstallerBuilderError("Binary package is not defined in pyproject.toml at [tool.poexy.binary]")
    ...
    def build(
        ...
        dist_path = str(dist_path or self.__project_path / "dist")
        ...
        logger.info(f"Building executable:")
