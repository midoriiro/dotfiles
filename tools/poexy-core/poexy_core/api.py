...
def __get_wheel_builder(
    ...
    if poexy.wheel.format is not None:
        wheel_format = poexy.wheel.format
        ...
        elif len(wheel_format) == 1 and WheelFormat.Binary not in wheel_format:
            return wheel_builder
        elif len(wheel_format) == 1 and WheelFormat.Binary in wheel_format:
            return binary_builder
        else:
            raise PoexyBuilderError(
                "Wheel package format is not valid"
            )
...
def get_requires_for_build_wheel(
    ...
    logger.info(f"=== Get requires for build wheel called ===")
    logger.info(f"config_settings: {config_settings}")

    try:    
        pyproject = PyProjectTOML(path=Path().resolve())
        logger.info(f"Pyproject loaded successfully")
...
def prepare_metadata_for_build_wheel(
    ...
    logger.info(f"=== Prepare metadata for build wheel called ===")
    ...
    try:
        ...
        logger.info(f"Builder created successfully")
...
def build_wheel(
    ...
    logger.info(f"=== Build wheel called ===")
...
def build_sdist(
    ...
    logger.info(f"=== Build sdist called ===")
