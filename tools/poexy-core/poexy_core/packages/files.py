...
class PackageFile(BaseModel):
    source: Path = Field(
        description="Source path where file will be copied from"
    )
...
class ResolvedPackageFiles(BaseModel):
    includes: PackageFiles = Field(
        description="Includes files"
    )
    excludes: PackageFiles = Field(
        description="Excludes files"
    )

    @model_validator(mode="after")
    def validate(self) -> "ResolvedPackageFiles":
...
class FilePattern(BaseModel):
    ...
    path: Path = Field(
        description="Path where to match the files"
    )

    @model_validator(mode="after")
    def validate(self) -> "FilePattern":
