...
class MetadataManifestBuilder:
    ...
    def set(self, field: MetadataField, value: ManifestValue):
        if field.version > self.__version:
            raise ValueError(f"Field {field.name} is not supported in version {self.__version}")
        if field.deprecated_since and field.deprecated_since <= self.__version:
            raise ValueError(f"Field {field.name} is deprecated since {field.deprecated_since}")
    ...
    def from_poetry(builder: "MetadataManifestBuilder", metadata: PoetryMetadata):
        ...
        # TODO: Add license expression and license file
        # elif metadata.license_expression:
        #     builder.set(MetadataField.LicenseExpression, metadata.license_expression)
        # elif metadata.license_file:
        #     builder.set(MetadataField.LicenseFile, metadata.license_file)
        ...
        if metadata.project_urls:
            builder.set(MetadataField.ProjectURL, sorted(metadata.project_urls, key=lambda u: u[0]))

        # 2.1
        if metadata.description_content_type:
            builder.set(MetadataField.DescriptionContentType, metadata.description_content_type)
