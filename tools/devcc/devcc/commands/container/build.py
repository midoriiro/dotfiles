...
def command(
    ...
    try:
        ...
        # Store the feature in the context for further use
        if build_feature.has_valid_fields():
            container_feature = ctx.obj.features.get('container')
            if container_feature is None:
                container_feature = ContainerFeature()
            container_feature.build = build_feature
            ctx.obj.features['container'] = container_feature
