...
def test_workspace_command_with_name(app: App):
...
def test_workspace_command_with_volume(app: App):
...
def test_workspace_command_with_both(app: App):
...
def test_workspace_command_with_empty_name(app: App):
...
def test_workspace_command_with_short_name(app: App):
    result = runner.invoke(app.typer, ["workspace", "--name", "te"])
    assert result.exit_code == 2
    assert "Name must be at least 3 characters long" in result.output
    assert app.context.features.get("workspace") is None

def test_workspace_command_with_non_alphanumeric_name(app: App):
...
def test_workspace_command_with_empty_volume_name(app: App):
    result = runner.invoke(app.typer, ["workspace", "--volume-name", ""])
    assert result.exit_code == 2
    assert "Volume name cannot be empty" in result.output
    assert app.context.features.get("workspace") is None

def test_workspace_command_with_short_volume_name(app: App):
...
def test_workspace_command_with_non_alphanumeric_volume_name(app: App):
    result = runner.invoke(app.typer, ["workspace", "--volume-name", "test-volume"])
    assert result.exit_code == 2
    assert "Volume name must be alphanumeric" in result.output
    assert app.context.features.get("workspace") is None

def test_workspace_command_with_feature_creation_error(app: App, monkeypatch):
