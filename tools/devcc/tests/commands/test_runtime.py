...
def test_runtime_command_with_invalid_user_format(app: App, runner: CliRunner):
...
def test_runtime_command_with_invalid_user_type(app: App, runner: CliRunner):
    result = runner.invoke(app.typer, ["runtime", "--user", "invalid:testuser"])
    assert result.exit_code == 2
    assert "User must be in format 'container:user' or 'remote:user'" in result.output

def test_runtime_command_with_empty_user(app: App, runner: CliRunner):
    result = runner.invoke(app.typer, ["runtime", "--user", "remote:"])
    assert result.exit_code == 2
    assert "User cannot be empty" in result.output

def test_runtime_command_with_short_user(app: App, runner: CliRunner):
...
def test_runtime_command_with_non_alphanumeric_user(app: App, runner: CliRunner):
    result = runner.invoke(app.typer, ["runtime", "--user", "remote:user-name"])
    assert result.exit_code == 2
    assert "User must be alphanumeric" in result.output

def test_runtime_command_with_invalid_env_format(app: App, runner: CliRunner):
...
def test_runtime_command_with_invalid_env_type(app: App, runner: CliRunner):
    result = runner.invoke(app.typer, ["runtime", "--env", "invalid:key=value"])
    assert result.exit_code == 2
    assert "must be in 'container:key=value' or 'remote:key=value' format" in result.output

def test_runtime_command_with_empty_env(app: App, runner: CliRunner):
...
def test_runtime_command_with_feature_creation_error(app: App, runner: CliRunner, monkeypatch):
