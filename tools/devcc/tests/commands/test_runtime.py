...
def test_runtime_command_with_remote_user(app: App):
...
def test_runtime_command_with_container_user(app: App):
...
def test_runtime_command_with_container_env(app: App):
...
def test_runtime_command_with_remote_env(app: App):
...
def test_runtime_command_with_mount(app: App):
...
def test_runtime_command_with_mount_options(app: App):
...
def test_runtime_command_with_volume_mount(app: App):
...
def test_runtime_command_with_invalid_user_format(app: App):
    result = runner.invoke(app.typer, ["runtime", "--user", "invalid_format"])
    assert result.exit_code == 2
    assert "User must be in format 'container:user' or 'remote:user'" in result.output

def test_runtime_command_with_invalid_user_type(app: App):
    result = runner.invoke(app.typer, ["runtime", "--user", "invalid:testuser"])
    assert result.exit_code == 2
    assert "User must be in format 'container:user' or 'remote:user'" in result.output

def test_runtime_command_with_empty_user(app: App):
    result = runner.invoke(app.typer, ["runtime", "--user", "remote:"])
    assert result.exit_code == 2
    assert "User cannot be empty" in result.output

def test_runtime_command_with_short_user(app: App):
    result = runner.invoke(app.typer, ["runtime", "--user", "remote:ab"])
    assert result.exit_code == 2
    assert "User must be at least 3 characters long" in result.output

def test_runtime_command_with_root_user(app: App):
    result = runner.invoke(app.typer, ["runtime", "--user", "remote:root"])
    assert result.exit_code == 2
    assert "User cannot be 'root'" in result.output

def test_runtime_command_with_digit_start_user(app: App):
    result = runner.invoke(app.typer, ["runtime", "--user", "remote:1user"])
    assert result.exit_code == 2
    assert "User cannot start with a digit" in result.output

def test_runtime_command_with_non_alphanumeric_user(app: App):
    result = runner.invoke(app.typer, ["runtime", "--user", "remote:user-name"])
    assert result.exit_code == 2
    assert "User must be alphanumeric" in result.output 

def test_runtime_command_with_invalid_env_format(app: App):
    result = runner.invoke(app.typer, ["runtime", "--env", "invalid_format"])
    assert result.exit_code == 2
    assert "must be in 'container:key=value' or 'remote:key=value' format" in result.output

def test_runtime_command_with_invalid_env_type(app: App):
    result = runner.invoke(app.typer, ["runtime", "--env", "invalid:key=value"])
    assert result.exit_code == 2
    assert "must be in 'container:key=value' or 'remote:key=value' format" in result.output

def test_runtime_command_with_empty_env(app: App):
    result = runner.invoke(app.typer, ["runtime", "--env", " "])
    assert result.exit_code == 2
    assert "Environment variable string cannot be empty" in result.output

def test_runtime_command_with_env_no_equals(app: App):
    result = runner.invoke(app.typer, ["runtime", "--env", "container:keyvalue"])
    assert result.exit_code == 2
    assert "must be in key=value format" in result.output

def test_runtime_command_with_empty_env_key(app: App):
...
def test_runtime_command_with_empty_env_value(app: App):
...
def test_runtime_command_with_invalid_mount_format(app: App):
...
def test_runtime_command_with_invalid_mount_type(app: App):
...
def test_runtime_command_with_empty_mount(app: App):
    result = runner.invoke(app.typer, ["runtime", "--mounts", " "])
    assert result.exit_code == 2
    assert "Mount point string cannot be empty" in result.output

def test_runtime_command_with_empty_mount_source(app: App):
...
def test_runtime_command_with_empty_mount_target(app: App):
    result = runner.invoke(app.typer, ["runtime", "--mounts", "/host/path::bind"])
    assert result.exit_code == 2
    assert "Source, target, and type cannot be empty" in result.output

def test_runtime_command_with_empty_mount_type(app: App):
    result = runner.invoke(app.typer, ["runtime", "--mounts", "/host/path:/container/path:"])
    assert result.exit_code == 2
    assert "Source, target, and type cannot be empty" in result.output

def test_runtime_command_with_empty_mount_options(app: App):
    result = runner.invoke(app.typer, ["runtime", "--mounts", "/host/path:/container/path:bind:"])
    assert result.exit_code == 2
    assert "Options cannot be empty if provided" in result.output

def test_runtime_command_with_feature_creation_error(app: App, monkeypatch):
